...
// the output of our image transformation
...
)
{
    ...
    if ( fp )
    {
        printf ( "\n\n\t\t donnees de l'image height=%d, width=%d\n", *height, *width ) ;
...
int main( void )
{
    ...
    char* pgmInFileName[100]; //= "images/balloons.ascii.pgm" ;
    char* pgmOutFileName;// = "images/balloons_noisy1.ascii_out.pgm" ;
    //char* uuui[3];
    //uuui[0]='g';
    //printf("\n %d \n",strlen(uuui));
    printf("\t\tMerci d'utiliser notre programe de traitement d'images.");
    printf(" Entrer le chemin de l'image en pgm \n \tNB: Vous pouvez utiliser \" images/balloons.ascii.pgm \"");
    printf(" . la sorti sera dans \" image/out/MENU_UTILISER \"\n\t\t\t\t exemple de sorti \" image/out/convolve.pgm \" \n");
    //fgets(pgmInFileName, sizeof(pgmInFileName), stdin);  // read string
    scanf("%s",pgmInFileName);
    printf("%s",pgmInFileName);
    ...
    printf("\n\n with %d \n\n",width);
  //code to transform the image goes here ...
    int i,j;
    for(i = 0; i < 1025; i++){
    ...
    hid=height;
    wid=height;
    imageProcessing(theImageArray, theImageArrayDup,height,width,maxPixel,width,pgmOutFileName);
